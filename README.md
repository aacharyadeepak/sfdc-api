...
* SFDC_API - Code for the application's Lambda function.
...
```bash
sfdc-api$ sam build --use-container
```

The SAM CLI installs dependencies defined in `SFDC/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
sfdc-api$ sam local invoke SFDCFunction --event events/sfdc.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sfdc-api$ sam local start-api
sfdc-api$ curl http://localhost:3000/
```
...
```bash
sfdc-api$ sam logs -n SFDCFunction --stack-name "sfdc-api" --tail
```
...
```bash
sfdc-api$ pip install -r tests/requirements.txt --user
# unit test
sfdc-api$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sfdc-api$ AWS_SAM_STACK_NAME="sfdc-api" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "sfdc-api"
```
...
Next, you can use AWS Serverless Application Repository to deploy ready to use Apps that go beyond sfdc api samples and learn how authors developed their applications: [AWS Serverless Application Repository main page](https://aws.amazon.com/serverless/serverlessrepo/)